...
public class SaveLoadClickListener implements OnClickListener {
	...
	public void onClick(final View view) {
		String text = mFileSelector.getSelectedFileName();
		if (checkFileName(text)) {
			// Append file extension if not explicitly entered during SAVE operation
			if (mOperation == FileOperation.SAVE) {
				if (mFileSelector.fileTypeFilter.equals("*.*") == false) {
					if (text.endsWith(mFileSelector.fileTypeFilter) == false) {
						text = text + mFileSelector.fileTypeFilter;
					}
				}
			}
			// End append file extension
